...
def is_text_unreadable(text: str) -> bool:
    """
    Check if the extracted text is empty, garbled, scrambled, or otherwise unreadable.
    """
    ...
    # 1. Check for Private Use Area characters or replacement characters
    # Unicode Private Use Area: E000-F8FF, F0000-FFFFD, 100000-10FFFD
    pua_chars = sum(
    ...
    if pua_chars / total_len > 0.15:
        return True
        
    # 2. Check for ratio of alphanumeric characters/spaces to symbols
    alpha_num_chars = sum(1 for c in clean_text if c.isalnum() or c.isspace())
    if alpha_num_chars / total_len < 0.5:
        return True
        
    # 3. Check common European words for Latin-script text if long enough
    latin_chars = sum(1 for c in clean_text if 'a' <= c.lower() <= 'z')
    latin_ratio = latin_chars / total_len
    ...
    if latin_ratio > 0.4:
        ...
        if len(words) >= 5:
            common_words = {
                # English
                "the", "and", "of", "to", "in", "is", "that", "it", "on", "for", "as", "with", "was", "at", "by", "an", "be", "this", "are",
                # German
                "der", "die", "das", "und", "ist", "in", "zu", "den", "von",
                # French
                "le", "la", "les", "et", "en", "un", "une", "est", "dans",
                # Spanish
                "el", "la", "los", "y", "en", "un", "una", "es", "con"
            }
            matching_common = sum(1 for w in words if w in common_words)
            if matching_common == 0:
                return True
            ...
            with pdfplumber.open(pdf_bytes) as pdf:
                for page_num, page in enumerate(pdf.pages, 1):
                    ...
                    if ocr_service:
                        ...
                        if is_text_unreadable(text_content):
                            # Text cannot be read, is empty, or is scrambled. Run full page OCR!
                            try:
